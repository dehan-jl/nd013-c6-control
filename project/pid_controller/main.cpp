...
int main() {
  ...
  /**
   * DONE (Step 1): create pid (pid_throttle) for throttle command and
   *initialize values
   **/
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer,
    ...
    if (s != "") {
      ...
      // compute the angle between current position and planned position (needed
      // yaw) - current yaw
       ...
       **/
      // error = last planned velocity - current velocity
